                        ...
                        <div class="card-body bg-light text-dark py-4">
                            <form action="" method="POST">
                                <label for="username" class="form-label">Username</label>
                                    ...
                                    <div class= "row mb-3">
                                        <button type="submit" class="btn btn-primary btn-block" name="btnDaftar">Daftar</button>
                                    </div>     
                            </form>
                        </div>
...
if(isset($_POST['btnDaftar'])){
    ...
    if(empty($username) || empty($password)) {
      ...
      } elseif(count((array) $koneksi->query("SELECT username FROM tbl_user WHERE username = '" . $username . "'")->fetch_array()) > 1) {
        /**
         * Cek jika username atau email telah terdaftar.
         */
        echo '<script>alert("Username sudah ada");
        window.location.href = "regis.php";
        </script>';
      } else {
        $insert = mysqli_query($koneksi, "INSERT INTO tbl_user (username, password) VALUES ('" . $username . "', '" . $password . "')");
        if($insert){
